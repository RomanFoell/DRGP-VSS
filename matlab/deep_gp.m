...
if strcmp(config.simulation,'on')
    config.hyp = opt_hyp;
    [y_test,V_test] = deep_gp_regression_test(X_exogen_ref,B,config);
    [y_pred,V_pred,error_plot,error_l_1,error_l_2,error_l_inf] = plot_dynamic(y_ref,y_test,V_test,config);
end %if
