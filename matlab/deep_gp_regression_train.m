...
save('python/data/load_python.mat','X','y','lengthscale','lengthscale_p','sn','sf','S','MU_S','SIGMA_S','MU','SIGMA','MU_A','SIGMA_A','U','b','D','layers','order')
...
elseif strcmp(config.simulation,'on')
    ...
    for i = 1:config.layers+1
        ...
        if strcmp(config.version_variational_spectrum,'SV1')
            [statistics1,MU_S_hat] = feval(config.kernel_fun_statistics1,X_input_temp,SIGMA_inputs_quad,SIGMA_S,MU_S,Y_M_temp,k_config);
            statistics2 = feval(config.kernel_fun_statistics2,MU_S_hat,X_input_temp,SIGMA_inputs_quad,SIGMA_S,Y_M_temp,k_config);
        elseif  strcmp(config.version_variational_spectrum,'SV2')
            [statistics1,S_hat] = feval(config.kernel_fun_statistics1,X_input_temp,SIGMA_inputs_quad,S,Y_M_temp,k_config);
            statistics2 = feval(config.kernel_fun_statistics2,S_hat,X_input_temp,SIGMA_inputs_quad,Y_M_temp,k_config);
        end%if
        A_K_MN_NM_chol{i} = chol(statistics2 + snquad * eye(config.mm) +  jitter * eye(config.mm))';
