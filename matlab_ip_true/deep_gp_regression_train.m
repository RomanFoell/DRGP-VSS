...
save('python_IP_true/data/load_python.mat','X','y','lengthscale','lengthscale_p','sn','sf','S','MU_S','SIGMA_S','MU','SIGMA','MU_A','SIGMA_A','U','b','D','layers','order')
...
elseif strcmp(config.simulation,'on')
    ...
    title('initialized hidden states')
    legend('show')
    ...
    for i = 1:config.layers+1
        ...
        A_K_MN_NM_chol{i} = chol(statistics2 + snquad * eye(config.mm))';
