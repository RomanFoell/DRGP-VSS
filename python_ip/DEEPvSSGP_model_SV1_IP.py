...
#       M # 1
    ...
    def get_model(self, w, lengthscale_trf, lengthscale_p_trf, sn_trf, sf_trf, MU_S, SIGMA_S_trf, MU, SIGMA_trf, U, b, X, y, MEAN_MAP, Q, D, D_cum_sum, layers, order, non_rec, N, M):
        ...
        for i in range(0, layers + 1):
            ...
            EPhiTPhi_reg = self.reg_EPhi(lengthscale_trf[D_cum_sum[i]:D_cum_sum[i + 1]], lengthscale_p_trf[D_cum_sum[i]:D_cum_sum[i + 1]], sf_trf[i], MU_S[:,D_cum_sum[i]:D_cum_sum[i + 1]], X_inputs[:,D_cum_sum[i]:D_cum_sum[i + 1]], SIGMA_inputs[:,D_cum_sum[i]:D_cum_sum[i + 1]], U[:,D_cum_sum[i]:D_cum_sum[i + 1]], b[:,i], N, M, i, D[i], order, non_rec)
            if i == layers:
#                z = (1 + 2 * T.cos(w)**2) * y[order:]
            ...
            opt_A_mean, cholSigInv, cholK_MM, InvK_MM = self.get_opt_A(sn_trf[i], EEPhiTPhi, zT_EEPhi, K_MM)
                    
            LL = LL - 0.5 * N * T.log(sn_trf[i]) - 0.5 * N * np.log(2 * np.pi)  + 0.5 * T.sum(2 * T.log(T.diag(cholK_MM))) - 0.5 * T.sum(2 * T.log(T.diag(cholSigInv))) - 0.5 * T.sum(SIGMA_trf_LL)/sn_trf[i] - 0.5 * T.sum(z ** 2)/sn_trf[i] + 0.5 * T.sum(opt_A_mean.T * zT_EEPhi)/sn_trf[i]  - 0.5 * T.sum(N * sf_trf[i] - T.sum(T.diag(InvK_MM.dot(EPhiTPhi_reg))))/sn_trf[i]
