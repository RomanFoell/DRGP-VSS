...
#                z = (1 + 2 * T.cos(w)**2) * y[order:]
            ...
            LL = LL - 0.5 * (N - M) * T.log(sn_trf[i]) - 0.5 * N * np.log(2 * np.pi)  - 0.5 * T.sum(2 * T.log(T.diag(cholSigInv))) - 0.5 * T.sum(SIGMA_trf_LL)/sn_trf[i] - 0.5 * T.sum(z ** 2)/sn_trf[i] + 0.5 * T.sum(opt_A_mean.T * zT_EEPhi)/sn_trf[i]  - 0.5 * T.sum(N * sf_trf[i] - T.sum(T.diag(InvK_MM.dot(EPhiTPhi_reg))))/sn_trf[i]
