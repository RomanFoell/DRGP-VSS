...
opt_params = {'sn': sn, 'sf': sf, 'U': U,  'hyp': hyp, 'MU_S': MU_S, 'SIGMA_S': SIGMA_S, 'MU': MU, 'SIGMA': SIGMA} # optimized parameters
...
startTime = time()
#bnds = np.transpose(np.squeeze(np.stack((-3*np.ones((x0.shape[0],1)),5*np.ones((x0.shape[0],1)))),axis=2))
