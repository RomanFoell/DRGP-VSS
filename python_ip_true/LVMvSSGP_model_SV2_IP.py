...
#       M # 1
    ...
    def get_opt_A(self, sn_trf, EPhiTPhi, XT_EPhi, K_MM):
        cholSigInv = sT.cholesky(EPhiTPhi + (sn_trf + 1e-6)  * T.identity_like(K_MM))
    ...
    def get_model(self, lengthscale_trf, lengthscale_p_trf, sn_trf, sf_trf, S, MU, SIGMA_trf, U, b, X, Q, D, N, M):
        ...
        LL = - 0.5 * (D * ((N - M) * T.log(sn_trf) + N * np.log(2 * np.pi) + T.sum(2 * T.log(T.diag(cholSigInv))) + (N * sf_trf - T.sum(T.diag(InvK_MM.dot(EPhiTPhi_reg))))/sn_trf) + T.sum(X ** 2)/sn_trf -  T.sum(opt_A_mean.T * XT_EPhi)/sn_trf)
